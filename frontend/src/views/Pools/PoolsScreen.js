...
import {IBEP20,MasterChefV2}  from '../../hooks/abiHelpers'
import tokenAddress  from '../../hooks/address'
 ...
 const {accounts, isLoaded} = useContext(Web3Context)
  ...
  <>
    ...
    <div className="sm:-my-5 -my-3 sm:mx-0 lg:mr-4 md:p-0 p-8">
      <motion.div className="sm:-my-5 -my-3 mx-auto max-w-screen-xl "      initial={{ scale: 1.5, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.75 }}>
        <div className=" mt-8 rounded-md bg-gradient-to-br from-blue-secondary via-blue-700 to-blue-600 py-10" >      <div className="max-w-7xl mx-auto py-3 px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between flex-wrap">
          <div className="w-0 flex-1 flex items-center">
            <span className="flex p-2 rounded-lg ">
              <img src="/pool/maquina.svg" alt="" className="w-full text-white" aria-hidden="true" />
            </span>
            <p className="ml-3 font-medium text-white truncate">
              <span className="md:hidden text-xl">Pools</span>
              <span className="hidden md:inline text-8xl font-medium">Pools</span>
              <br/>
  <span className="md:hidden text-xl">Just Stake some tokens to earn</span>
 <span className="hidden md:inline text-xl pt-2">Just Stake some tokens to earn</span>
         <br/>
  <span className="md:hidden text-xl">High APR, low risk</span>
 <span className="hidden md:inline text-xl pt-2">High APR, low risk</span>
            </p>

          </div>

          <div className="flex justify-center">
            <img src="/pool/bg-pools.svg" alt="" className="absolute justify-center mr-[1120px] -mt-24 hidden md:block"/> 
          </div>
        </div>
      </div>
    </div>
...
<motion.div className=" px-4 py-5  sm:px-6 mt-12  max-w-6xl mx-auto"   initial="initial"
          animate="visible"
          variants={{
            initial: { opacity: 0 },
        ...
        <div className="ml-4 mt-4">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              > <img src="/farm/Grupo_14497.svg" className="w-6.5" alt=""  onClick={handleTabSwitch}
                  activeTab={activeTab === 1}
                  id={1}
/></button>
            </div>
   <div className="ml-4">
      ...
      <Switch.Label as="span" className="ml-3 hidden md:block">
        <span className="text-sm font-medium text-gray-500 font-semibold">Staked Only </span>
...
<option>{pool.name}</option>
   ...
   onChange={e=>setSearchTerm(e.target.value)}
      ...
      <div className={`${!activeTab ? '' : 'hidden'}`} activeTab={activeTab === 0}>
        <CardPools  activepool={activepool} searchTerm={searchTerm} isLoaded={isLoaded}/>
      </div>
   
      <div className={`${activeTab ? '' : 'hidden'}`} activeTab={activeTab === 1}>
